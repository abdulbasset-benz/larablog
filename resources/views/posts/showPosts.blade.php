            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ($posts as $post)
                    <div class="bg-white shadow-lg rounded-lg p-6 transition transform hover:-translate-y-1 hover:shadow-2xl">
                        ...
                        <a href="{{ route('posts.show', ['post' => $post->id]) }}" class="mt-5 bg-violet-500 text-white px-5 py-2 rounded-md inline-block hover:bg-violet-600 font-semibold">Read more</a>
                    </div>
                @endforeach
            </div>
        </div>
